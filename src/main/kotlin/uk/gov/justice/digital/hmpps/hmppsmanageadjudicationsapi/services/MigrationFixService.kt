      ...
      .forEach { record ->
        ...
        nextHearingAfter?.let {
          it.hearingOutcome?.let { hearingOutcome ->
            if (listOf(Finding.PROVED.name, Finding.NOT_PROCEED.name, Finding.D.name).contains(hearingOutcome.details) && hearingOutcome.code == HearingOutcomeCode.ADJOURN && record.hearings.getOrNull(policeReferIdx + 2) == null) {
