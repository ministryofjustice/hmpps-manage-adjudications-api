...
class HearingOutcomeServiceTest : ReportedAdjudicationTestBase() {
  ...
  inner class DeleteHearingOutcome {
    ...
    fun `delete adjourn outcome calls prison api to update hearing`() {
      ...
      verify(eventWrapperService, atLeastOnce()).amendHearing(
        reportedAdjudication.reportNumber,
        OicHearingRequest(reportedAdjudication.hearings.first().oicHearingId, reportedAdjudication.hearings.first().dateTimeOfHearing, reportedAdjudication.hearings.first().oicHearingType, reportedAdjudication.hearings.first().locationId),
      )
