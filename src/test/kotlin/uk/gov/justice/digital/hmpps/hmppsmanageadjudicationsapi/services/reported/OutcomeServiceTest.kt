...
class OutcomeServiceTest : ReportedAdjudicationTestBase() {
  ...
  override fun `throws an entity not found if the reported adjudication for the supplied id does not exists`() {
    ...
    Assertions.assertThatThrownBy {
      outcomeService.repair("1", OutcomeCode.CHARGE_PROVED, OutcomeCode.NOT_PROCEED)
    }.isInstanceOf(EntityNotFoundException::class.java)
      .hasMessageContaining("ReportedAdjudication not found for 1")
  }
  ...
  inner class Repair {
    ...
    @Test
    fun `throws exception if no such outcome present`() {
      whenever(reportedAdjudicationRepository.findByChargeNumber(any())).thenReturn(
        entityBuilder.reportedAdjudication().also {
          it.clearOutcomes()
        },
      ...
      Assertions.assertThatThrownBy {
        outcomeService.repair(chargeNumber = "", correctOutcomeCode = OutcomeCode.NOT_PROCEED, incorrectOutcomeCode = OutcomeCode.CHARGE_PROVED)
      }.isInstanceOf(ValidationException::class.java)
        .hasMessageContaining("no such outcome ${OutcomeCode.NOT_PROCEED}")
    }

    @CsvSource("REFER_INAD", "REFER_POLICE", "REFER_GOV", "QUASHED", "PROSECUTION")
    @ParameterizedTest
    fun `throws exception if outcome repair not supported`(outcomeCode: OutcomeCode) {
      whenever(reportedAdjudicationRepository.findByChargeNumber(any())).thenReturn(
        entityBuilder.reportedAdjudication().also {
          it.clearOutcomes()
        },
      ...
      Assertions.assertThatThrownBy {
        outcomeService.repair(chargeNumber = "", correctOutcomeCode = outcomeCode, incorrectOutcomeCode = OutcomeCode.CHARGE_PROVED)
      }.isInstanceOf(ValidationException::class.java)
        .hasMessageContaining("not supported for repair $outcomeCode")
    }

    @Test
    fun `currently charge proved followed by not proceed, not proceed is correct, adjourns charge proved and removes punishments`() {
      val argumentCaptor = ArgumentCaptor.forClass(ReportedAdjudication::class.java)
      whenever(reportedAdjudicationRepository.findByChargeNumber(any())).thenReturn(
        entityBuilder.reportedAdjudication().also {
          it.hearings.clear()
          ...
          it.addOutcome(
            Outcome(id = 2, code = OutcomeCode.CHARGE_PROVED).also { it.createDateTime = LocalDateTime.now().minusDays(1) },
          )
          ...
          it.addOutcome(
            Outcome(id = 1, code = OutcomeCode.NOT_PROCEED).also { it.createDateTime = LocalDateTime.now() },
          )
      ...
      outcomeService.repair(chargeNumber = "1", correctOutcomeCode = OutcomeCode.NOT_PROCEED, incorrectOutcomeCode = OutcomeCode.CHARGE_PROVED)
      verify(reportedAdjudicationRepository).save(argumentCaptor.capture())
    ...
    @Test
    fun `currently not proceed followed by charge proved, charge proved is correct, adjourns not proceed and preserves any punishments`() {
      val argumentCaptor = ArgumentCaptor.forClass(ReportedAdjudication::class.java)
      whenever(reportedAdjudicationRepository.findByChargeNumber(any())).thenReturn(
        entityBuilder.reportedAdjudication().also {
          it.hearings.clear()
          ...
          it.addOutcome(
            Outcome(id = 2, code = OutcomeCode.NOT_PROCEED).also { it.createDateTime = LocalDateTime.now().minusDays(1) },
          )
          ...
          it.addOutcome(
            Outcome(id = 1, code = OutcomeCode.CHARGE_PROVED).also { it.createDateTime = LocalDateTime.now() },
          )
      ...
      outcomeService.repair(chargeNumber = "1", correctOutcomeCode = OutcomeCode.CHARGE_PROVED, incorrectOutcomeCode = OutcomeCode.NOT_PROCEED)
      verify(reportedAdjudicationRepository).save(argumentCaptor.capture())
    ...
    @Test
    fun `currently charge proved followed by not proceed, charge proved is correct, deletes not proceed and preserves punishments`() {
      val argumentCaptor = ArgumentCaptor.forClass(ReportedAdjudication::class.java)
      whenever(reportedAdjudicationRepository.findByChargeNumber(any())).thenReturn(
        entityBuilder.reportedAdjudication().also {
          it.hearings.clear()
    ...
    @Test
    fun `currently not proceed followed by charge proved, not proceed is correct, removes charge proved and removes any punishments`() {
      val argumentCaptor = ArgumentCaptor.forClass(ReportedAdjudication::class.java)
      whenever(reportedAdjudicationRepository.findByChargeNumber(any())).thenReturn(
        entityBuilder.reportedAdjudication().also {
          it.hearings.clear()
          ...
          it.addOutcome(
            Outcome(id = 2, code = OutcomeCode.NOT_PROCEED, details = "some").also { it.createDateTime = LocalDateTime.now().minusDays(1) },
          )
    ...
    @Test
    fun `refer inad, scheduled hearing, charge proved, not proceed, should only alter the vharge proved record`() {
      val argumentCaptor = ArgumentCaptor.forClass(ReportedAdjudication::class.java)
      whenever(reportedAdjudicationRepository.findByChargeNumber(any())).thenReturn(
        entityBuilder.reportedAdjudication().also {
          it.hearings.clear()
          ...
          it.hearings.add(
            Hearing(
              ...
              chargeNumber = "",
              oicHearingType = OicHearingType.GOV_ADULT,
              dateTimeOfHearing = LocalDateTime.now().minusDays(2),
              locationId = 1,
              agencyId = "",
          ...
          it.addOutcome(
            Outcome(id = 3, code = OutcomeCode.REFER_INAD, details = "some").also { it.createDateTime = LocalDateTime.now().minusDays(3) },
          )

          it.addOutcome(
            Outcome(id = 4, code = OutcomeCode.SCHEDULE_HEARING, details = "some").also { it.createDateTime = LocalDateTime.now().minusDays(2) },
          )
          ...
          it.addOutcome(
            Outcome(id = 2, code = OutcomeCode.CHARGE_PROVED, details = "some").also { it.createDateTime = LocalDateTime.now().minusDays(1) },
          )
      ...
      assertThat(argumentCaptor.value.getOutcomes().first().code).isEqualTo(OutcomeCode.REFER_INAD)
      assertThat(argumentCaptor.value.getOutcomes()[1].code).isEqualTo(OutcomeCode.SCHEDULE_HEARING)
      assertThat(argumentCaptor.value.getOutcomes().last().code).isEqualTo(OutcomeCode.NOT_PROCEED)
