...
class HearingServiceTest : ReportedAdjudicationTestBase() {
  ...
  inner class CreateHearing {
    ...
    fun init() {
      ...
      whenever(eventWrapperService.createHearing(any(), any())).thenReturn("5")
    }
  ...
  inner class AmendHearing {
    ...
    fun `amend a hearing`() {
      ...
      val argumentCaptor = ArgumentCaptor.forClass(ReportedAdjudication::class.java)
      verify(reportedAdjudicationRepository).save(argumentCaptor.capture())
      verify(eventWrapperService, atLeastOnce()).amendHearing(
        ADJUDICATION_NUMBER,
        OicHearingRequest(
          oicHearingId = "3",
  ...
  inner class DeleteHearing {
    ...
    fun `delete a hearing`() {
      ...
      val argumentCaptor = ArgumentCaptor.forClass(ReportedAdjudication::class.java)
      verify(reportedAdjudicationRepository).save(argumentCaptor.capture())
      verify(eventWrapperService, atLeastOnce()).deleteHearing(ADJUDICATION_NUMBER, "3")
    ...
    fun `delete a hearing when there is more than one and ensure status is still SCHEDULED`() {
      ...
      val argumentCaptor = ArgumentCaptor.forClass(ReportedAdjudication::class.java)
      verify(reportedAdjudicationRepository).save(argumentCaptor.capture())
      verify(eventWrapperService, atLeastOnce()).deleteHearing(ADJUDICATION_NUMBER, "2")
    ...
    fun `delete hearing removes last outcome if it was SCHEDULE HEARING`() {
      whenever(reportedAdjudicationRepository.findByReportNumber(any())).thenReturn(
        entityBuilder.reportedAdjudication()
          .also {
            it.hearings.add(
              Hearing(agencyId = "", locationId = 1L, oicHearingType = OicHearingType.INAD_ADULT, dateTimeOfHearing = LocalDateTime.now().plusDays(5), oicHearingId = "1", reportNumber = "1"),
            )
      ...
      val argumentCaptor = ArgumentCaptor.forClass(ReportedAdjudication::class.java)
      verify(reportedAdjudicationRepository).save(argumentCaptor.capture())
      verify(eventWrapperService, atLeastOnce()).deleteHearing(ADJUDICATION_NUMBER, "1")
