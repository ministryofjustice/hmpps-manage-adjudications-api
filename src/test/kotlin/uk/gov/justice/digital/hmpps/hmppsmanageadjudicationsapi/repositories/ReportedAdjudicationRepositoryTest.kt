...
class ReportedAdjudicationRepositoryTest {
  ...
  @Test
  fun `findAdjudicationsForPrisonerWithPunishments excludes deleted ADA punishments`() {
    // Create an adjudication with both active and deleted ADA punishments
    val adjudication = reportedAdjudicationRepository.save(
      entityBuilder.reportedAdjudication(offenderBookingId = 1L, chargeNumber = "ADA_TEST").also {
        it.hearings.clear()
        it.status = ReportedAdjudicationStatus.CHARGE_PROVED
        it.dateTimeOfDiscovery = LocalDateTime.now().minusDays(1)
        it.prisonerNumber = "A9962AE"
        
        // Add an active ADA punishment
        it.addPunishment(
          entityBuilder.punishment(type = PunishmentType.ADDITIONAL_DAYS).also { punishment ->
            punishment.deleted = null // Active punishment
          }
        )
        
        // Add a deleted ADA punishment
        it.addPunishment(
          entityBuilder.punishment(type = PunishmentType.ADDITIONAL_DAYS).also { punishment ->
            punishment.deleted = true // Deleted punishment
          }
        )
      }
    )

    // Query for adjudications with ADA punishments
    val response = reportedAdjudicationRepository.findAdjudicationsForPrisonerWithPunishments(
      prisonerNumber = "A9962AE",
      startDate = LocalDateTime.now().minusYears(1),
      endDate = LocalDateTime.now(),
      statuses = listOf(ReportedAdjudicationStatus.CHARGE_PROVED.name),
      ada = true,
    ...
    // Should return only one result (not duplicated due to deleted punishment)
    assertThat(response.content.size).isEqualTo(1)
    
    // Verify the returned adjudication only has active punishments
    val returnedAdjudication = response.content.first()
    assertThat(returnedAdjudication.getPunishments().size).isEqualTo(1)
    assertThat(returnedAdjudication.getPunishments().first().deleted).isNotEqualTo(true)
  }

  @Test
  fun `findAdjudicationsForPrisonerWithPunishments returns no results when all ADA punishments are deleted`() {
    // Create an adjudication with only deleted ADA punishments
    val adjudication = reportedAdjudicationRepository.save(
      entityBuilder.reportedAdjudication(offenderBookingId = 1L, chargeNumber = "ADA_ALL_DELETED").also {
        it.hearings.clear()
        it.status = ReportedAdjudicationStatus.CHARGE_PROVED
        it.dateTimeOfDiscovery = LocalDateTime.now().minusDays(1)
        it.prisonerNumber = "A9962AE"
        
        // Add only deleted ADA punishments
        it.addPunishment(
          entityBuilder.punishment(type = PunishmentType.ADDITIONAL_DAYS).also { punishment ->
            punishment.deleted = true // Deleted punishment
          }
    ...
    // Should return no results since all ADA punishments are deleted
